...
```console
docker run cisagov/gatherer:1.5.12-rc.1
```
...
1. Create a `docker-compose.yml` file similar to the one below to use [Docker Compose](https://docs.docker.com/compose/).
    ...
    services:
      gatherer:
        image: cisagov/gatherer:1.5.12-rc.1
        volumes:
          - type: bind
...
1. Pull the new image:
    ...
    ```console
    docker pull cisagov/gatherer:1.5.12-rc.1
    ```
...
|-----------|-------------|
|`cisagov/gatherer:1.5.12-rc.1`| An exact release version. |
...
```console
docker build \
  --build-arg VERSION=1.5.12-rc.1 \
  --tag cisagov/gatherer:1.5.12-rc.1 \
