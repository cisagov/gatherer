...
[![License](https://img.shields.io/github/license/cisagov/gatherer)](https://spdx.org/licenses/)
...
```console
docker run cisagov/gatherer:2.0.2-rc.1
```
...
1. Create a `compose.yml` file similar to the one below to use [Docker Compose](https://docs.docker.com/compose/).
    ...
    services:
      gatherer:
        image: cisagov/gatherer:2.0.2-rc.1
        volumes:
...
1. Pull the new image:
    ...
    ```console
    docker pull cisagov/gatherer:2.0.2-rc.1
    ```
...
|-----------|-------------|
|`cisagov/gatherer:2.0.2-rc.1`| An exact release version. |
...
```console
docker build \
  --tag cisagov/gatherer:2.0.2-rc.1 \
